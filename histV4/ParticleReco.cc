  ...
  // mass hypotheses for the decay products
    ...
    for ( i = 0; i < Ev.nParticles(); i++ ) {
        ...
        if ( Ev.particle(i)->q == 1 ) {
            ...
            somma_E_Lambda0 += Utilities::energy( Ev.particle( i )->mx, Ev.particle( i )->my, Ev.particle( i )->mz, c.Proton() );
        }
        if ( Ev.particle(i)->q == -1 ) {
            
            somma_E_Lambda0 += Utilities::energy( Ev.particle( i )->mx, Ev.particle( i )->my, Ev.particle( i )->mz, c.Pion() );
    ...
    if ( positive != 1 || negative != 1) return;
